...
struct mllama_hparams {
    ...
    std::vector<uint32_t> intermediate_layers;
};
...
static ggml_cgraph *mllama_image_build_graph(mllama_ctx *ctx, const mllama_image_batch *imgs) {
    ...
    // encoder
    struct ggml_tensor *intermediate_embd = ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, hparams.intermediate_layers.size(), hidden_size, (num_positions + num_padding_patches) * num_tiles);
    auto intermediates = hparams.intermediate_layers;
    for (size_t il = 0; il < model.layers.size(); il++) {
        auto it = std::find(intermediates.begin(), intermediates.end(), il);
        if (it != intermediates.end()) {
            auto s = std::distance(intermediates.begin(), it);
...
struct mllama_ctx *mllama_model_load(const char *fname, const int verbosity = 1) {
    ...
    hparams.intermediate_layers = get_u32_array(ctx, "mllama.vision.intermediate_layers");
